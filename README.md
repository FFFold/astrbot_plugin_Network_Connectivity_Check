# 网络连接监测

AstrBot 网络连接监测插件 - 定时监测网络连接状态，支持 HTTP/Ping/TCP 多种检测方式，可用于代理连通性、服务可用性监测

## 功能特性

- **多种检测方式**：支持 HTTP 请求、ICMP Ping、TCP 连接三种检测方式
- **定时监测**：可配置检测间隔，自动后台运行
- **智能通知**：
  - 状态变化时通知（可配置连续失败阈值，防误报）
  - 每次检测都通知（调试用）
  - 支持夜间免打扰时段
- **灵活配置**：支持多个目标同时监测，可在 WebUI 中以可视化方式配置
- **历史记录**：保存检测历史，便于查看网络状况趋势
- **详细错误**：检测失败时提供具体错误信息（如超时、连接被拒绝等）

## 安装方法

1. 在 AstrBot WebUI 中打开「插件」页面
2. 点击「安装插件」
3. 输入仓库地址：`https://github.com/FFFold/astrbot_plugin_Network_Connectivity_Check`
4. 安装完成后重启 AstrBot

## 使用方法

### 指令列表

| 指令 | 用法 | 说明 |
|------|------|------|
| `/net check` | `/net check [目标名]` | 手动执行一次检测，不指定目标则检测所有目标 |
| `/net status` | `/net status` | 查看所有监测目标的当前状态 |
| `/net history` | `/net history <目标名> [数量]` 或 `/net history <目标名> <开始时间> <结束时间>` | 查看指定目标的检测历史 |
| `/net addme` | `/net addme [描述]` | 将当前聊天添加到通知目标列表 |

### 配置说明

在 AstrBot WebUI 的插件配置页面中进行配置：

#### 监测目标 (targets)

配置需要监测的网络目标列表。使用 `template_list` 类型，在 WebUI 中以卡片形式展示，方便添加和管理：

- **名称**：用于标识该目标（如 Google、百度）
- **URL/地址**：要检测的网址或 IP
- **检测方式**：`http` / `ping` / `tcp`
- **使用自定义检测参数**：是否为该目标单独设置检测参数（覆盖全局设置）
  - 启用后可单独设置：检测间隔、超时时间、失败重试次数

#### 检测全局设置 (detection_settings)

所有监测目标的默认检测参数：

- **检测间隔**：两次检测之间的间隔（秒），默认 300 秒
- **超时时间**：单次检测的最大等待时间（秒），默认 10 秒
- **失败重试次数**：检测失败时的重试次数，默认 3 次
- **验证 HTTPS 证书**：HTTP 检测时是否验证 TLS/SSL 证书，默认开启。关闭后可检测自签名证书网站，但存在中间人攻击风险

#### 通知全局设置 (notification_settings)

检测结果的通知策略，**四个开关相互独立**：

##### 1. 状态变化时通知 (notify_on_status_change)
- **默认**：开启
- **作用**：当网络状态发生变化时推送通知
- **特殊逻辑**：
  - 从异常**恢复**时：立即通知，不受阈值限制
  - 从正常变为**异常**时：需达到"连续失败阈值"才通知（防误报）
- **推荐使用方式**：日常使用时开启此选项，关闭其他两个

##### 2. 状态变化通知的连续失败阈值 (consecutive_failures)
- **默认**：2 次
- **作用**：仅影响"状态变化时通知"中"从正常变为异常"的场景
- **用途**：避免偶发性网络波动导致的误报

##### 3. 每次成功都通知 (notify_on_success)
- **默认**：关闭
- **作用**：每次检测成功都发送通知
- **注意**：会刷屏！建议仅在调试时开启
- **不受阈值限制**

##### 4. 每次失败都通知 (notify_on_failure)
- **默认**：关闭
- **作用**：每次检测失败都发送通知
- **注意**：会刷屏！建议仅在调试时开启
- **不受阈值限制**

##### 5. 免打扰设置
- **免打扰开始/结束时间**：设置夜间不发送通知的时间段
- **设为 -1**：关闭免打扰功能

#### 通知逻辑示例

假设阈值为 2，只开启"状态变化时通知"：

```
检测1：成功  → 首次检测，通知 ✅（状态：未知→正常）
检测2：失败  → 连续失败=1，未达阈值，不通知
检测3：失败  → 连续失败=2，达到阈值，通知 ✅（状态：正常→异常）
检测4：失败  → 状态未变化，不通知
检测5：成功  → 状态恢复，立即通知 ✅（状态：异常→正常）
```

#### 通知目标 (notify_targets)

配置检测结果通知给哪些用户或群组。使用 `template_list` 类型，在 WebUI 中以卡片形式展示：

- **UMO**：统一消息来源标识
- **描述**：用于标识此通知目标

**快速添加方法**：在群里或私聊中发送 `/net addme`，即可自动将当前聊天添加到通知列表。

UMO 格式示例：
- 群聊：`aiocqhttp:GroupMessage:123456`
- 私聊：`aiocqhttp:FriendMessage:123456`

#### 高级设置 (advanced_settings)

- **历史记录保留数**：每个目标保留的最近检测记录数量（默认 100）

## 示例配置

### 监测目标

添加两个监测目标：

```json
[
  {
    "name": "Google",
    "url": "https://www.google.com",
    "method": "http",
    "custom_settings": false
  },
  {
    "name": "百度",
    "url": "https://www.baidu.com",
    "method": "http",
    "custom_settings": false
  }
]
```

### 检测全局设置

```json
{
  "interval": 300,
  "timeout": 10,
  "retry": 3
}
```

### 通知全局设置

```json
{
  "notify_on_status_change": true,
  "notify_on_success": false,
  "notify_on_failure": false,
  "consecutive_failures": 2,
  "silent_hours_start": -1,
  "silent_hours_end": 7
}
```

### 通知目标

```json
[
  {
    "umo": "aiocqhttp:GroupMessage:123456789",
    "description": "我的QQ群"
  },
  {
    "umo": "aiocqhttp:FriendMessage:987654321",
    "description": "管理员私聊"
  }
]
```

## 使用场景

1. **服务器监控**：监测服务器是否能访问外网/国际互联网
2. **网站可用性**：监测网站是否正常提供服务
3. **网络质量**：通过响应时间和历史记录分析网络状况

## 工作原理

1. 插件启动后，会为每个监测目标创建一个后台任务
2. 按照设定的间隔自动执行检测
3. 检测成功/失败时更新目标状态
4. 根据通知策略决定是否发送通知：
   - 状态变化时通知（异常需满足阈值）
   - 每次检测都通知（调试用）
5. 所有检测记录保存到历史，便于后续分析

## 常见问题

**Q: 为什么网络已经异常了但没有收到通知？**

A: 请检查：
1. 是否开启了"状态变化时通知"或"每次失败都通知"
2. 如果开启了"状态变化时通知"，连续失败次数是否达到了阈值
3. 当前是否处于免打扰时段

### 历史时间范围查询

`/net history` 除了支持查看最近 N 条记录，也支持按时间范围筛选。

说明：

- 按数量查询时，最多支持查看最近 50 条记录
- 按时间范围查询时，最多展示最近 50 条匹配记录，并会在返回内容中明确提示

示例：

```text
/net history Google 10
/net history Google 2026-04-01 2026-04-07
/net history Google 2026-04-01T00:00:00 2026-04-07T23:59:59
```

支持的时间格式：

- `YYYY-MM-DD`
- `YYYY-MM-DDTHH:MM:SS`
- `YYYY-MM-DD HH:MM:SS`（如果平台命令解析支持带空格参数）

如果只写日期：

- 开始时间按当天 `00:00:00`
- 结束时间按当天 `23:59:59`

**Q: 为什么收到了很多重复通知？**

A: 请检查：
1. 是否误开启了"每次成功都通知"或"每次失败都通知"
2. 日常使用时建议只开启"状态变化时通知"

## 支持与文档

- [AstrBot 主仓库](https://github.com/AstrBotDevs/AstrBot)
- [AstrBot 插件开发文档](https://docs.astrbot.app/dev/star/plugin-new.html)

## 许可证

MIT License
